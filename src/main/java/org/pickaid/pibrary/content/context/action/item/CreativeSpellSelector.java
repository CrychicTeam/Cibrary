...
import dev.xkmc.Pibrary.init.registrate.LMItems;
...
public class CreativeSpellSelector extends IItemSelector {
	...
	private static List<ResourceLocation> getSpells(RegistryAccess level) {
		return level.registryOrThrow(LibraryRegistries.SPELL).holders()
				.sorted(Comparator.<Holder.Reference<Action>>comparingInt(e -> e.get().order())
	...
	public List<ItemStack> getDisplayList() {
		var level = Proxy.getWorld();
		assert level != null;
		return getListGeneric(rl -> level.registryAccess().registryOrThrow(LibraryRegistries.SPELL).get(rl).icon().getDefaultInstance()
				.setHoverName(Component.translatable(Action.lang(rl))));
	}
