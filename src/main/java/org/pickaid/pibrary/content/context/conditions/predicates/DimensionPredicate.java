...
public record DimensionPredicate(List<ResourceKey<Level>> dimensions) {
	private static final Codec<ResourceKey<Level>> DIMENSION_CODEC = CodecUtils.resourceKeyCodec(Registries.DIMENSION);
	private static final Codec<List<ResourceKey<Level>>> DIMENSIONS_CODEC = CodecUtils.resourceKeyCodec(Registries.DIMENSION).listOf();
