...
import dev.xkmc.Pibrary.init.data.LMLangData;
...
public class SpellCastCommand extends MagicCommandEventHandlers {
	...
	public static LiteralArgumentBuilder<CommandSourceStack> build() {
		return literal("cast").requires(e -> e.hasPermission(1))
				.then(argument("user", EntityArgument.entities())
						.then(Commands.argument("skill", ResourceKeyArgument.key(LibraryRegistries.SPELL))
								.then(literal("instant")
										.executes(ctx -> run(ctx, ActionCastType.INSTANT, false))
										.then(argument("power", DoubleArgumentType.doubleArg(0))
												.executes(ctx -> run(ctx, ActionCastType.INSTANT, true))
										)
								).then(literal("charge")
										.then(argument("chargeTime", IntegerArgumentType.integer(0))
												.executes(ctx -> run(ctx, ActionCastType.CHARGE, false))
												.then(argument("power", DoubleArgumentType.doubleArg(0))
														.executes(ctx -> run(ctx, ActionCastType.CHARGE, true))
												)
										)
								).then(literal("continuous")
										.then(argument("duration", IntegerArgumentType.integer(0))
												.executes(ctx -> run(ctx, ActionCastType.CONTINUOUS, false))
												.then(argument("power", DoubleArgumentType.doubleArg(0))
														.executes(ctx -> run(ctx, ActionCastType.CONTINUOUS, true))
												)
										)
								)
						)
	...
	private static int run(CommandContext<CommandSourceStack> ctx, ActionCastType type, boolean usePower) throws CommandSyntaxException {
		var list = EntityArgument.getEntities(ctx, "user");
		var holder = resolveKey(ctx, "skill", LibraryRegistries.SPELL, ERR_INVALID_NAME);
		...
		int time = 0;
		if (type == ActionCastType.CONTINUOUS) {
			time = IntegerArgumentType.getInteger(ctx, "duration");
		}
		if (type == ActionCastType.CHARGE) {
