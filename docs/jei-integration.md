...
public final class ExampleJeiCategory<R> implements IRecipeCategory<PiJeiRecipeDisplay<R>> {
    ...
    public void setRecipe(
        ...
        for (PiRecipeSlot<?> slot : display.layout().slots()) {
            ...
            slot.tooltips().forEach(line -> jeiSlot.addTooltipCallback((recipeSlotView, tooltip) ->
                    tooltip.add(line)));
...
That covers rotating ingredients, hidden lookup ingredients, linked input/output
variants, and extra tooltips while keeping rendering details in the JEI adapter.
